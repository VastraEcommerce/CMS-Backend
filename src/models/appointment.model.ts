...
type AppointmentModelQuery = Query<
  any,
  HydratedDocument<IAppointment>,
  AppointmentQueryHelpers
> &
  AppointmentQueryHelpers;
interface AppointmentQueryHelpers {
  byDay(this: AppointmentModelQuery, date: string): AppointmentModelQuery;
  byStartEndDate(
    this: AppointmentModelQuery,
    Startdate: any,
    endDate: any
...
>(
  {
    ...
    appointmentNumber: {
      type: Number,
      requied: [true, 'Appointment number must be provided'],
    },
  },
  {
    timestamps: true,
...
schema.static(
  ...
  async function isAvailable(appointment: IAppointment) {
    ...
    return (await Appointment.findOne({ doctor, appointmentNumber })
      .byDay(date as unknown as string)
      .countDocuments())
      ? false
      : true;
  }
